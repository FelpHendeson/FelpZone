...
> **Estado:** implementado e consolidado nas Fatias 24.1 a 24.7.
...
A versão definitiva do schema é `schemaVersion: 18`. `GameState.calendar` guarda só `consumedEventIds`. Calendário, origens, estágios e eventos ficam no pack `world/calendar.json`. A idade é derivada da origem; a UI nunca envia data nem idade. Fronteiras (`day|cycle|season|year.started`) nascem depois de `advanceDayCycle` em `executeSandboxAction`. Recarga não duplica ocorrência consumida.

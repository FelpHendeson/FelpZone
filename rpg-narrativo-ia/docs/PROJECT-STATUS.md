...
O [Dia 2 — especificação narrativa e jogável](DAY-2-NARRATIVE-SPEC.md) e a [implementação do Dia 2](DAY-2-IMPLEMENTATION-SPEC.md) estão especificados. As **Fatias A–E estão implementadas**: Atividades Contextuais usam schema 26; a Margem Rochosa, as pistas e Caio/Davi compõem o mundo; `day-two-others` reconhece as rotas herdadas de Mira; e `escort-davi-to-clearing` executa tempo, participantes e relocação persistente. A próxima etapa é a Fatia F — Autonomia e encerramento.
...
| Camada de UI/UX jogável | **UI/UX 2.0 implementada; apresentação em protótipo** | Hierarquia “aventura primeiro, dados sob demanda”: HUD compacto, cena e decisões contextuais; cinco destinos inferiores (`Mundo`, `Jornadas`, `Personagem`, `Mochila`, `Menu`); telas focadas para mapa, pessoas, progressão, Registro, sociedade e domínio. | Ícones e imagens ainda são placeholders; a reorganização não adiciona regras ao domínio. Consulte [UI/UX 2.0](UI-UX-2-NAVIGATION.md). |
| Sistema 8 — Presenças e interações | **Implementado e consolidado** | Catálogo, sincronização, planejamento, `PresenceState` no schema 4, `presence.interact`, interface mobile e conteúdo jogável. No Dia 1 revisado, sinais humanos antecedem Mira e o jogador pode observar, conversar ou evitar contato; o coelho chifrudo continua observável/evitável sem combate. | Sem IA conversacional ou simulação contínua própria; agenda fica no Sistema 17. |
...
**Os Sistemas 1 a 29 estão implementados e consolidados.** O Dia 1 fechou no schema 25. A Fatia A do Dia 2 adicionou Atividades Contextuais e avançou o save para **schema 26** sem criar Sistema 30. As Fatias B–E adicionaram mapa, sobreviventes, jornada, primeiro contato, atividades de Davi e a tensão social da Nascente sem alterar o schema. O próximo trabalho é a Fatia F — Autonomia e encerramento.

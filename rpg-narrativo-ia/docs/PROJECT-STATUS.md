...
| Sistema 12 — Banco de ações e combate | **Núcleo implementado e consolidado (12.1–12.7); integração especificada (12.8–12.12)** | Catálogos, motor determinístico, IA por regras, habilidades liberando ações e encontro jogável. O próximo recorte já define descoberta da ameaça, saúde persistente, custo temporal único, consequências atômicas e consolidação visual. | As Fatias 12.8–12.12 ainda não estão implementadas; posicionamento, status ricos, grupos, balanceamento definitivo e itens seguem fora do escopo. |
...
As integrações de descoberta, saúde persistente, custo temporal e apresentação foram definidas pelo autor e especificadas nas Fatias 12.8 a 12.12, mas ainda aguardam implementação. Posicionamento, iniciativa por frações de turno, condições de status ricas, grupos, itens e balanceamento definitivo permanecem fora do escopo. Consulte [Sistema 12 — Banco de ações e combate](SYSTEM-ACTION-COMBAT.md) e [Consolidação do Sistema 12](SYSTEM-12-CONSOLIDATION.md).
...
**Os Sistemas 1 a 12 possuem núcleo implementado e consolidado.** O Sistema 11 entregou o núcleo diegético do Sistema (progressão no schema 7); o Sistema 12 entregou o banco de ações e o combate por turnos determinístico. A próxima meta aprovada para especificação é consolidar a integração do combate nas Fatias 12.8 a 12.12. Consulte [Sistema 11](SYSTEM-ETERIS-NUMEN-PROGRESSION.md), [Sistema 12](SYSTEM-ACTION-COMBAT.md) e [Consolidação do Sistema 12](SYSTEM-12-CONSOLIDATION.md).

Antes da próxima implementação:

1. implementar somente após autorização explícita as Fatias 12.8 a 12.12 já especificadas;

...
O fechamento da revisão do Dia 1 está em [Dia 1 — Playtest da Fatia G](docs/DAY-1-PLAYTEST.md). O Dia 2 está especificado em [Dia 2 — narrativa](docs/DAY-2-NARRATIVE-SPEC.md) e [Dia 2 — implementação](docs/DAY-2-IMPLEMENTATION-SPEC.md). As **Fatias A–E estão implementadas**: [Atividades Contextuais](docs/MECHANIC-CONTEXTUAL-ACTIVITIES.md) usa schema 26; a Margem Rochosa, as pistas e Caio/Davi já existem; `day-two-others` cobre primeiro contato e afastamento; `escort-davi-to-clearing` é a primeira atividade jogável, com tempo, participantes, relocação, guidance e narrativa. A próxima implementação é a Fatia F — Autonomia e encerramento.

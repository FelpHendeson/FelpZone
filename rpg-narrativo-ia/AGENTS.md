...
O fechamento da revisão do Dia 1 está em [Dia 1 — Playtest da Fatia G](docs/DAY-1-PLAYTEST.md). O Dia 2 está especificado em [Dia 2 — narrativa](docs/DAY-2-NARRATIVE-SPEC.md) e [Dia 2 — implementação](docs/DAY-2-IMPLEMENTATION-SPEC.md). A **Fatia A está implementada**: [Atividades Contextuais](docs/MECHANIC-CONTEXTUAL-ACTIVITIES.md) usa schema 26, catálogo de pack, `activity.perform`, participantes validados pelo motor, UI e guidance; não é Sistema 30. O catálogo ativo está vazio até o conteúdo de Caio/Davi. A próxima implementação é a Fatia B — Mundo do Dia 2.

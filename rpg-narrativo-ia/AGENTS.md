...
O fechamento da revisão do Dia 1 está em [Dia 1 — Playtest da Fatia G](docs/DAY-1-PLAYTEST.md). O Dia 2 está especificado em [Dia 2 — narrativa](docs/DAY-2-NARRATIVE-SPEC.md) e [Dia 2 — implementação](docs/DAY-2-IMPLEMENTATION-SPEC.md). As **Fatias A–C estão implementadas**: [Atividades Contextuais](docs/MECHANIC-CONTEXTUAL-ACTIVITIES.md) usa schema 26; a Margem Rochosa, as pistas e Caio/Davi já existem; `day-two-others` e as interações permitem observar, abordar ou evitar os sobreviventes com variantes do estado herdado de Mira. O catálogo ativo de atividades continua vazio até a Fatia D. A próxima implementação é Davi e a primeira atividade.

...
O fechamento da revisão do Dia 1 está em [Dia 1 — Playtest da Fatia G](docs/DAY-1-PLAYTEST.md). O Dia 2 está especificado em [Dia 2 — narrativa](docs/DAY-2-NARRATIVE-SPEC.md) e [Dia 2 — implementação](docs/DAY-2-IMPLEMENTATION-SPEC.md). As **Fatias A–B estão implementadas**: [Atividades Contextuais](docs/MECHANIC-CONTEXTUAL-ACTIVITIES.md) usa schema 26, catálogo de pack, `activity.perform`, UI e guidance; a Margem Rochosa, as pistas do Dia 2 e Caio/Davi com agendas e presenças já existem no pack. O catálogo ativo de atividades continua vazio até a Fatia D. A próxima implementação é a Fatia C — Jornada e primeiro contato.

...
export class ImmutableIndex<K, V> implements ReadonlyMap<K, V> {
  ...
  get [Symbol.toStringTag](): string { return 'ConditionIndex'; }
  set(key: K, value: V): never {
    void key;
    void value;
    throw new ConditionError('O índice de condições é imutável.');
  }
  delete(key: K): never {
    void key;
    throw new ConditionError('O índice de condições é imutável.');
  }
  clear(): never { throw new ConditionError('O índice de condições é imutável.'); }
}
